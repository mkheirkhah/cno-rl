...
def central_agent(net_params_queues, exp_queues,
    ...
    with tf.Session() as sess, open(LOG_FILE + '_test', 'wb') as test_log_file:
        ...
        actor = rl_a3c.ActorNetwork(sess,
                                         state_dim=[S_INFO, S_LEN], action_dim=dimension,
                                         learning_rate=alr)
        critic = rl_a3c.CriticNetwork(sess,
                                           state_dim=[S_INFO, S_LEN],
                                           learning_rate=clr)

        summary_ops, summary_vars = rl_a3c.build_summaries()
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue,
    ...
    net_env = rl_env.Environment(all_cooked_time=all_cooked_time,
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'w') as log_file, \
        ...
        while True:  # experience video streaming forever
            ...
            entropy_record.append(rl_a3c.compute_entropy(action_prob[0]))
