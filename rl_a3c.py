...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            split_2 = tflearn.conv_1d(inputs[:, 1:2, :],
                                      128,
                                      4,
                                      activation='relu')
            # past chunk download time (array)
            split_3 = tflearn.conv_1d(inputs[:, 2:3, :],
                                      128,
                                      4,
                                      activation='relu')
    ...
    def set_network_params(self, input_network_params):
        self.sess.run(self.set_network_params_op,
                      feed_dict={
                          i: d
                          for i, d in zip(self.input_network_params,
...
class CriticNetwork(object):
    ...
    def get_gradients(self, inputs, td_target):
        return self.sess.run(self.critic_gradients,
                             feed_dict={
                                 self.inputs: inputs,
                                 self.td_target: td_target
                             })
